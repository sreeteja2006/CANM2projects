...
import numpy as np
import os
from question1 import q1_solver
...
def run_case(name, F, Fy, Fyp, domain, BC, N, tol, max_iter, verbose):
    ...
    w, iters, hist = solver.solve_fdm(tol=tol, max_iter=max_iter, verbose=verbose)
...
for name, F, Fy, Fyp, domain, BC in cases:
    if name == "Group-1":
        result = q1_solver.question_1_solver(N, domain, F, BC, Fy=Fy, Fyp=Fyp, tol=tol, max_iter=max_iter)
        if len(result) == 3:
            w, iters, hist = result
        else:
            w, hist = result
            iters = len(hist["res_2"])
    else:
...
res_lists = pad_to_same_length([all_hist[n][2]["res_2"] for n in names])
...
plt.title("Residual vs iteration (Groups 1--6)")
plt.grid(True)
plt.legend()
safe_savefig("outputs/plots/residual_vs_iteration_G1toG6.png")
...
plt.title("Condition number vs iteration (Groups 1--6)")
plt.grid(True)
plt.legend()
safe_savefig("outputs/plots/cond_vs_iteration_G1toG6.png")
...
plt.title("Smallest singular value vs iteration (Groups 1--6)")
plt.grid(True)
plt.legend()
safe_savefig("outputs/plots/sigma_min_vs_iteration_G1toG6.png")

for name in names:
    w, iters, hist = all_hist[name]
    final_res = float(hist["res_2"][-1])
    ...
    print(f"{name}: iterations={iters}, ||F||2={final_res:.3e}, sigma_min={final_sig:.3e}, cond2={final_cond:.3e}, diag_dom={dd_final}")
